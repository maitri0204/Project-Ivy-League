...
function Pointer5Content() {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`http://localhost:5000/api/pointer5/status/${studentId}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Pointer 5: Essay Score</h1>
        ...
        <div className="space-y-6">
          {loading ? (
            ...
            <div className="border border-gray-200 rounded-lg p-6 bg-green-50">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Essay Evaluation</h2>
              <div className="space-y-4">
                <div>
                  <p className="text-sm font-medium text-gray-700 mb-1">Score</p>
                  <p className="text-3xl font-bold text-green-800">{status.evaluation.score}/10</p>
                </div>
            ...
            <div className="border border-gray-200 rounded-lg p-6">
              <p className="text-gray-500 text-center">
                {status?.essay
                  ? 'Essay has been submitted. Waiting for counselor evaluation.'
                  : 'No essay evaluation available yet.'}
              </p>
            </div>
          )}
